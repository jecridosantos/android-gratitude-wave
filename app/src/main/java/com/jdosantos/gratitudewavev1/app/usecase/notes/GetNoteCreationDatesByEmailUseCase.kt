...
import com.jdosantos.gratitudewavev1.core.common.util.getMonthsBetweenDates
...
class GetNoteCreationDatesByEmailUseCase @Inject constructor(
    ...
    fun execute(callback: (List<Date>) -> Unit) {
        noteRepository.getNoteCreationDatesByEmail({
            callback(it)
        }) {
    ...
    fun generateCalendar(callback: (List<CalendarToShow>) -> Unit) {
        execute {
